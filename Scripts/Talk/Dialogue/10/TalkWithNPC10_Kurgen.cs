...
public class TalkWithNPC10_Kurgen : MonoBehaviour
{
    ...
    public void StartTalk()                                           //Konusma basladýgýnda olacaklar
    {
        ...
        {
            if (speecCount == 0)
                GetComponent<KurgenDialog10>().FirsWords();
            BText[0].GetComponent<TextMeshProUGUI>().text = GetComponent<KurgenDialog10>().easyText[0];
            BText[1].GetComponent<TextMeshProUGUI>().text = GetComponent<KurgenDialog10>().easyText[1];
            BText[2].GetComponent<TextMeshProUGUI>().text = GetComponent<KurgenDialog10>().easyText[2];
        }
    ...
    public void FinishedTalk()                                        //konusma bittiginde konusma ekraný ayarlarýný sýfýrlar
    {
        ...
        speecCount = GetComponent<KurgenDialog10>().speecCountOut;
        ...
        GetComponent<KurgenDialog10>().speecCountOut = speecCount;
    }
    ...
    public void EB(int ButonNo)
    {
        if (PlayerController.WhatIsNameToTalkIam == transform.name)//bu olmazsa bütün konusma scriptleri ayný anda calýsýyor
        {
            PlayerController.isTalking = true;
            speecCount = GetComponent<KurgenDialog10>().speecCountOut;
            GetComponent<KurgenDialog10>().EB(ButonNo, speecCount);
            canIPassTalk = false;

            NPCTalk.GetComponent<TextMeshPro>().text = GetComponent<KurgenDialog10>().easyText[4];
            NPCTalk.GetComponent<TextMeshPro>().DOFade(1, 0.3f).SetDelay(0.6f);
            if (GetComponent<KurgenDialog10>().easyText[3] == null)
                playerTalk.GetComponent<TextMeshPro>().text = BText[ButonNo - 1].GetComponent<TextMeshProUGUI>().text;
            else if (GetComponent<KurgenDialog10>().easyText[3] == "null")
                playerTalk.GetComponent<TextMeshPro>().text = null;
            else
                playerTalk.GetComponent<TextMeshPro>().text = GetComponent<KurgenDialog10>().easyText[3];
    ...
    public void ForwardSpeech()
    {
        ...
        {
            speecCount = GetComponent<KurgenDialog10>().speecCountOut;
            GetComponent<KurgenDialog10>().ForwardSpeech(speecCount);
            NPCTalk.GetComponent<TextMeshPro>().text = null;
            playerTalk.GetComponent<TextMeshPro>().text = null;
            NPCTalk.GetComponent<TextMeshPro>().DOFade(0, 0f);
            playerTalk.GetComponent<TextMeshPro>().DOFade(0, 0f);
            ...
            if (GetComponent<KurgenDialog10>().easyText[3] == null & PlayerController.isTalking)
            {
                electionPanel.GetComponent<RectTransform>().DOScale(1, 0f);
                electionPanel.GetComponent<CanvasGroup>().DOFade(0.75f, 0.15f);
                for (int i = 0; i < BText.Length; i++)
                    BText[i].GetComponent<TextMeshProUGUI>().text = GetComponent<KurgenDialog10>().easyText[i];
            }
            else if (!GetComponent<KurgenDialog10>().npcTalkFirst)
            {
