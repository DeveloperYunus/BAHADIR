...
public class TalkWithNPC5_Witch : MonoBehaviour
{
    ...
    public void StartTalk()                                           //Konusma basladýgýnda olacaklar
    {
        ...
        {
            if (speecCount == 0)
                GetComponent<WitchDialog5>().FirsWords();
            BText[0].GetComponent<TextMeshProUGUI>().text = GetComponent<WitchDialog5>().easyText[0];
            BText[1].GetComponent<TextMeshProUGUI>().text = GetComponent<WitchDialog5>().easyText[1];
            BText[2].GetComponent<TextMeshProUGUI>().text = GetComponent<WitchDialog5>().easyText[2];
        }
    ...
    public void FinishedTalk()                                        //konusma bittiginde konusma ekraný ayarlarýný sýfýrlar
    {
        ...
        GetComponent<WitchDialog5>().speecCountOut = speecCount;
    }
    ...
    public void EB(int ButonNo)
    {
        ...
        {
            PlayerController.isTalking = true;
            speecCount = GetComponent<WitchDialog5>().speecCountOut;
            GetComponent<WitchDialog5>().EB(ButonNo, speecCount);
            canIPassTalk = false;

            NPCTalk.GetComponent<TextMeshPro>().text = GetComponent<WitchDialog5>().easyText[4];
            NPCTalk.GetComponent<TextMeshPro>().DOFade(1, 0.3f).SetDelay(0.6f);
            if (GetComponent<WitchDialog5>().easyText[3] == null)
                playerTalk.GetComponent<TextMeshPro>().text = BText[ButonNo - 1].GetComponent<TextMeshProUGUI>().text;
            else if (GetComponent<WitchDialog5>().easyText[3] == "null")
                playerTalk.GetComponent<TextMeshPro>().text = null;
            else
                playerTalk.GetComponent<TextMeshPro>().text = GetComponent<WitchDialog5>().easyText[3];
    ...
    public void ForwardSpeech()
    {
        ...
        {
            speecCount = GetComponent<WitchDialog5>().speecCountOut;
            GetComponent<WitchDialog5>().ForwardSpeech(speecCount);
            NPCTalk.GetComponent<TextMeshPro>().text = null;
            playerTalk.GetComponent<TextMeshPro>().text = null;
            NPCTalk.GetComponent<TextMeshPro>().DOFade(0, 0f);
            playerTalk.GetComponent<TextMeshPro>().DOFade(0, 0f);
            ...
            if (GetComponent<WitchDialog5>().easyText[3] == null & PlayerController.isTalking)
            {
                electionPanel.GetComponent<RectTransform>().DOScale(1, 0f);
                electionPanel.GetComponent<CanvasGroup>().DOFade(0.75f, 0.15f);
                for (int i = 0; i < BText.Length; i++)
                    BText[i].GetComponent<TextMeshProUGUI>().text = GetComponent<WitchDialog5>().easyText[i];
            }
            else if (!GetComponent<WitchDialog5>().npcTalkFirst)
            {
    ...
    public void ForwardSpeechFromScreen()
    {
        if (PlayerController.isTalking && canIPassTalk && PlayerController.WhatIsNameToTalkIam == transform.name)
        {
            ...
            canIPassTalk = false;
            StartCoroutine(PassTalk(1f));
            if (GetComponent<WitchDialog5>().threeQuestion >= 3)//silinecek
            {
                playerTalk.GetComponent<TextMeshPro>().text = GetComponent<WitchDialog5>().stringForOut;
