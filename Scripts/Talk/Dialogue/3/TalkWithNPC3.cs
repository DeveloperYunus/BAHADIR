...
public class TalkWithNPC3 : MonoBehaviour
{
    ...
    private void OnTriggerEnter2D(Collider2D other)
    {
        ...
        if (other.CompareTag("Player") && (Vector2.SqrMagnitude(other.transform.position - transform.position) > 3f))
        {
            if (!lookNPCface)
            {
                FinishedTalk();
    ...
    public void StartTalk()                                           //Konusma basladýgýnda olacaklar
    {
        ...
        {
            if (speecCount == 0)
                GetComponent<OldManTalk3>().FirsWords();
            BText[0].GetComponent<TextMeshProUGUI>().text = GetComponent<OldManTalk3>().easyText[0];
            BText[1].GetComponent<TextMeshProUGUI>().text = GetComponent<OldManTalk3>().easyText[1];
            BText[2].GetComponent<TextMeshProUGUI>().text = GetComponent<OldManTalk3>().easyText[2];
        }
    }
    ...
    public void EB(int ButonNo)
    {
        ...
        {
            PlayerController.isTalking = true;
            speecCount = GetComponent<OldManTalk3>().speecCountOut;
            GetComponent<OldManTalk3>().EB(ButonNo, speecCount);
            canIPassTalk = false;

            //Bu kýsmý playerTalk.GetComponent<TextMeshPro>().text, yerine bunu yaptýk ==>  BText[ButonNo - 1].GetComponent<TextMeshProUGUI>().text easyText[3]
            //sorunuun çözmek için

            NPCTalk.GetComponent<TextMeshPro>().text = GetComponent<OldManTalk3>().easyText[4];
            NPCTalk.GetComponent<TextMeshPro>().DOFade(1, 0.3f).SetDelay(0.6f);
            if (GetComponent<OldManTalk3>().easyText[3] == null)
                BText[ButonNo - 1].GetComponent<TextMeshProUGUI>().text = BText[ButonNo - 1].GetComponent<TextMeshProUGUI>().text;
            else if (GetComponent<OldManTalk3>().easyText[3] == "null")
                BText[ButonNo - 1].GetComponent<TextMeshProUGUI>().text = null;
            else
                BText[ButonNo - 1].GetComponent<TextMeshProUGUI>().text = GetComponent<OldManTalk3>().easyText[3];
    ...
    public void ForwardSpeech()
    {
        ...
        {
            speecCount = GetComponent<OldManTalk3>().speecCountOut;
            GetComponent<OldManTalk3>().ForwardSpeech(speecCount);
            NPCTalk.GetComponent<TextMeshPro>().text = null;
            playerTalk.GetComponent<TextMeshPro>().text = null;
            NPCTalk.GetComponent<TextMeshPro>().DOFade(0, 0f);
            playerTalk.GetComponent<TextMeshPro>().DOFade(0, 0f);
            ...
            for (int i = 0; i < BText.Length; i++) BText[i].GetComponent<TextMeshProUGUI>().text = null;
            ...
            if (GetComponent<OldManTalk3>().easyText[3] == null & PlayerController.isTalking)
            {
                electionPanel.GetComponent<RectTransform>().DOScale(1, 0f);
                electionPanel.GetComponent<CanvasGroup>().DOFade(0.75f, 0.15f);
                for (int i = 0; i < BText.Length; i++)
                    BText[i].GetComponent<TextMeshProUGUI>().text = GetComponent<OldManTalk3>().easyText[i];
            }
            else if (!GetComponent<OldManTalk3>().npcTalkFirst)
            {
    ...
    public void ForwardSpeechFromScreen()
    {
        if (PlayerController.isTalking && canIPassTalk && Vector2.SqrMagnitude(transform.position - player.transform.position) < 50)
        {
            ...
            canIPassTalk = false;
            StartCoroutine(PassTalk(1f));
            if (GetComponent<OldManTalk3>().threeQuestion >= 3)//silinecek
            {
                playerTalk.GetComponent<TextMeshPro>().text = GetComponent<OldManTalk3>().stringForOut;
    ...
    {       
        yield return new WaitForSeconds(WaithSeconds);
        playerTalk.GetComponent<TextMeshPro>().text = GetComponent<OldManTalk3>().easyText[3];
        NPCTalk.GetComponent<TextMeshPro>().text = GetComponent<OldManTalk3>().easyText[4];
    }
}
